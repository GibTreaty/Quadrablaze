...
namespace UIWidgets
{
	...
	public class EasyLayoutBridge : ILayoutBridge
	{
		...
		EasyLayoutNamespace.EasyLayout Layout;
		...
		public EasyLayoutBridge(EasyLayoutNamespace.EasyLayout layout, RectTransform defaultItem, bool updateContentSizeFitter=true)
		{
			Layout = layout;
		...
		{
			Layout.Stacking = isHorizontal ? EasyLayoutNamespace.Stackings.Vertical : EasyLayoutNamespace.Stackings.Horizontal;
			...
			if (Layout.LayoutType==EasyLayoutNamespace.LayoutTypes.Grid)
			{
				if (Layout.GridConstraint!=EasyLayoutNamespace.GridConstraints.Flexible)
				{
					Layout.GridConstraint = (isHorizontal) ? EasyLayoutNamespace.GridConstraints.FixedRowCount : EasyLayoutNamespace.GridConstraints.FixedColumnCount;
				}
			}
		...
		public void SetFiller(float first, float last)
		{
			var padding = IsHorizontal
				? new EasyLayoutNamespace.Padding(first, last, 0, 0)
				: new EasyLayoutNamespace.Padding(0, 0, first, last);
		...
		{
			if (Layout.LayoutType==EasyLayoutNamespace.LayoutTypes.Compact)
			{
				if (Layout.CompactConstraint==EasyLayoutNamespace.CompactConstraints.MaxColumnCount)
				{
					var c = Mathf.Max(1, Layout.CompactConstraintCount);
					return Mathf.Min(c, columns);
				}
			}
			else if (Layout.LayoutType==EasyLayoutNamespace.LayoutTypes.Grid)
			{
				if (Layout.GridConstraint==EasyLayoutNamespace.GridConstraints.FixedColumnCount)
				{
		...
		public int RowsConstraint(int rows)
		{
			if (Layout.LayoutType==EasyLayoutNamespace.LayoutTypes.Compact)
			{
				if (Layout.CompactConstraint==EasyLayoutNamespace.CompactConstraints.MaxRowCount)
				{
					var c = Mathf.Max(1, Layout.CompactConstraintCount);
			...
			{
				if (Layout.GridConstraint==EasyLayoutNamespace.GridConstraints.FixedRowCount)
				{
					return Mathf.Max(1, Layout.GridConstraintCount);
				}
			}

...
namespace UIWidgets
{
		...
		where TComponent : ListViewItem
	{
		...
		protected override bool CanOptimize()
		{
			...
			var validLayout = currentLayout is EasyLayoutNamespace.EasyLayout;
		...
		protected override void CalculateMaxVisibleItems()
		{
			...
			var lowest_width = ItemSizes.Count > 0 ? ItemSizes.Values.Min(x => x.x) : 0;
			var lowest_height = ItemSizes.Count > 0 ? ItemSizes.Values.Min(x => x.y) : 0;
		...
		{
			var block = Mathf.FloorToInt((float)index / (float)ItemsPerBlock());

			return Mathf.Max(0, BlockSizes.GetRange(0, block).SumFloat() + (LayoutBridge.GetSpacing() * (block - 1)));
		}
		...
		{
			return GetItemPosition(index) + GetItemSize(DataSource[index]) - LayoutBridge.GetSpacing() + LayoutBridge.GetMargin() - GetScrollSize();
		}
		...
		{
			return GetItemPositionBottom(index + 1);
		}

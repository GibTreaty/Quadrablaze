...
namespace UIWidgets
{
	...
	public class ListViewCustom<TComponent,TItem> : ListViewBase where TComponent : ListViewItem
	{
		...
		public EasyLayoutNamespace.EasyLayout Layout {
			get {
				return layout as EasyLayoutNamespace.EasyLayout;
			}
		}
		...
		protected ILayoutBridge LayoutBridge {
			get {
				if ((layoutBridge==null) && (CanOptimize()))
				{
					...
					if (layout is EasyLayoutNamespace.EasyLayout)
					{
						layoutBridge = new EasyLayoutBridge(layout as EasyLayoutNamespace.EasyLayout, DefaultItem.transform as RectTransform, setContentSizeFitter);
		...
		protected virtual void SetDirection(ListViewDirection newDirection, bool isInited = true)
		{
			...
			if (CanOptimize() && (layout is EasyLayoutNamespace.EasyLayout))
			{
				LayoutBridge.IsHorizontal = IsHorizontal();
		...
		protected virtual bool CanOptimize()
		{
			...
			var validLayout = currentLayout ? ((currentLayout is EasyLayoutNamespace.EasyLayout) || (currentLayout is HorizontalOrVerticalLayoutGroup)) : false;
